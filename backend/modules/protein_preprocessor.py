...
class ProteinPreprocessor:
    ...
    def _clean_generated_pdbqt(self):
        """Clean PDBQT generated by OpenBabel: strip null bytes, torsion REMARKs,
        ROOT/ENDROOT/TORSDOF, BRANCH/ENDBRANCH blocks, and embedded water molecules.
        Keep only ATOM/HETATM lines."""
        try:
            raw = self.output_pdbqt.read_bytes()
            ...
            if not has_root and not has_null and not has_remark:
                return
            ...
            if cleaned:
                ...
                print(f"[INFO] Cleaned PDBQT: {len(lines)} -> {len(cleaned)} lines")
        except Exception:
            pass

    def _clean_pdbqt_file(self, pdbqt_path: Path) -> bool:
        """Remove null bytes, invalid leading lines, and water-molecule ROOT blocks
        from OpenBabel PDBQT output. Returns True if the file was cleaned."""
        try:
            ...
            # Step 1: Remove leading invalid lines
            valid_tokens = ('ATOM', 'HETATM', 'REMARK', 'ROOT', 'END', 'BRANCH', 'ENDBRANCH', 'TORSDOF', 'BEGIN_RES', 'END_RES')
            first_valid = 0
            for i, line in enumerate(lines):
                if line.lstrip().startswith(valid_tokens):
                    first_valid = i
                    break
            if first_valid > 0:
                lines = lines[first_valid:]

            # Step 2: Remove ROOT/ENDROOT blocks containing water (HOH/WAT) residues
            # Keep only the first ROOT block (main protein) or non-water ROOT blocks
            water_residues = {'HOH', 'WAT', 'DOD', 'H2O', 'TIP'}
            cleaned_lines = []
            in_root = False
            root_lines = []
            root_has_water = False
            root_count = 0
            ...
            for line in lines:
                ...
                if in_root:
                    ...
                    if line.startswith('ENDROOT'):
                        in_root = False
                        root_count += 1
                        # Keep first non-water root (protein), skip water roots
                        if not root_has_water or root_count == 1:
                            cleaned_lines.extend(root_lines)
                        root_lines = []
                        continue
                else:
                    cleaned_lines.append(line)

            # Handle case where ROOT block wasn't closed
            if in_root:
                cleaned_lines.extend(root_lines)

            if has_null or first_valid > 0 or len(cleaned_lines) != len(lines):
                cleaned_text = '\n'.join(cleaned_lines)
                pdbqt_path.write_text(cleaned_text, encoding='utf-8')
                removed = len(lines) - len(cleaned_lines)
                print(f"[INFO] Cleaned PDBQT: null_bytes={has_null}, leading={first_valid}, removed_lines={removed}")
                return True
            return False
        except Exception as e:
            print(f"[WARN] PDBQT cleanup failed: {e}")
            return False
    ...
    def _is_valid_pdbqt(self, pdbqt_path: Path) -> bool:
        ...
        if not pdbqt_path.exists() or pdbqt_path.stat().st_size < 100:
            return False
        # Auto-clean null bytes and invalid leading lines
        self._clean_pdbqt_file(pdbqt_path)
        valid_types = {
    ...
    def prepare_with_openbabel(self, receptor_pdb: Path) -> Optional[str]:
        ...
        for i, cmd in enumerate(strategies, 1):
            try:
                ...
                subprocess.run(cmd, check=True, capture_output=True, text=True, timeout=300)
                # Clean PDBQT: remove null bytes, ROOT/ENDROOT, torsion REMARKs, waters
                self._clean_generated_pdbqt()
                if self._is_valid_pdbqt(self.output_pdbqt):
    ...
    def prepare_with_openbabel_python(self, receptor_pdb: Path) -> Optional[str]:
        """Try OpenBabel Python bindings directly without calling subprocess."""
        try:
            from openbabel import openbabel
            ob_conversion = openbabel.OBConversion()
            ob_conversion.SetInAndOutFormats("pdb", "pdbqt")
            mol = openbabel.OBMol()
            if ob_conversion.ReadFile(mol, str(receptor_pdb)):
                # Add hydrogens
                mol.AddHydrogens()
                # Compute charges
                try:
                    ...
                    pass
                if ob_conversion.WriteFile(mol, str(self.output_pdbqt)):
                    if self._is_valid_pdbqt(self.output_pdbqt):
                        print(f"[INFO]  Prepared via OpenBabel Python Bindings: {self.output_pdbqt}")
                        return str(self.output_pdbqt)
            return None
        except Exception as e:
            print(f"[WARN] OpenBabel Python Bindings failed: {e}")
            return None
    ...
    def process(self, interactive: bool = False, default_policy: Optional[Dict[str,bool]] = None) -> Optional[str]:
        ...
        print(f"     Ions/Metals: {summary['ions']} atoms")
        ...
        if not interactive:
            ...
            print(f"     Ions: {'KEEP' if policy['keep_ions'] else 'REMOVE'} ({summary['ions']} found)")
        ...
        for method in [self.prepare_with_mgltools, self.prepare_with_openbabel, self.prepare_with_openbabel_python, self.prepare_with_pdb2pqr_fallback]:
